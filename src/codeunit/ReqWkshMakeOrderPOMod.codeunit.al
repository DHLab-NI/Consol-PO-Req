...
codeunit 50104 "Req. Wksh.-Make Order2"
{
    ...
    var
        ...
        // Add PrevSONo to check if Source SO number has changed
        PrevSONo: Code[20];
    ...
    begin
        ...
        case ReqLine."Replenishment System" of
            // SGH Replenishment System: Transfer not applicable.
            ...
            ReqLine."Replenishment System"::Purchase, ReqLine."Replenishment System"::"Prod. Order":
                case ReqLine."Action Message" of
                    // SGH Action message: Cancel not applicable
                    /*                    ReqLine."Action Message"::Cancel:
                                            begin
                                                CarryOutAction.DeleteOrderLines(ReqLine);
                                                OrderCounter := OrderCounter + 1;
                    ...
                    // SGH Action message: New or blank is applicable
                    ReqLine."Action Message"::New, ReqLine."Action Message"::" ":
                        begin
    ...
    begin
        ...
        if CheckInsertFinalizePurchaseOrderHeader(ReqLine2, PurchOrderHeader, true) then begin
            ...
            //SGH update per SO No.
            PrevSONo := ReqLine2."Sales Order No.";
    ...
    begin
        ...
        /*  SGH        ShouldValidateSellToCustNo := ReqLine2."Sell-to Customer No." <> '';
    ...
    begin
        ...
        if IsHandled then
            exit;
        // SGH never update PO Ship-to code with Customer (SO) Ship-to code
    ...
    begin
        if ReqTemplate.Recurring then begin
            ...
            // SGH            ReqLine2.SetRange("Sell-to Customer No.", PurchOrderHeader."Sell-to Customer No.");
            // SGH            ReqLine2.SetRange("Ship-to Code", PurchOrderHeader."Ship-to Code");
            ...
            // SGH SGHTEST add PrevSOPONo to filters
            ReqLine2.SetRange("Sales Order No.", PrevSONo);
            IsHandled := false;
    ...
    begin
        ...
        //SGHTEST add Previous SO No. to text
        PrevSONo := TryPrevSONo;
    ...
    procedure GetTryParam(var TryPurchOrderHeader: Record "Purchase Header"; var TryLineCount: Integer; var TryNextLineNo: Integer; var TryPrevPurchCode: Code[10]; var TryPrevAddToPONo: Code[20]; var TryPrevSONo: Code[20]; var TryPrevShipToCode: Code[10]; var TryPrevLocationCode: Code[10]; var TryOrderCounter: Integer; var TryOrderLineCounter: Integer)
    begin
    ...
    // If true then new PO is created (default is false)
    var
        SalesLine: Record "Sales Line";
        Purchasing: Record Purchasing;
        IsHandled: Boolean;
    begin
        ...
        // If true then new PO is created
        // Default return is false
        // SGH Message('Result is %1 (true = new PO) for Item %2, sales order %3', Result, SalesLine."No.", SalesLine."Document No.");
    end;
    ...
    begin
        ...
        CheckInsert :=
              (PurchOrderHeader."Buy-from Vendor No." <> RequisitionLine."Vendor No.") or // If true then new PO is created
                                                                                          // SGH             (PurchOrderHeader."Sell-to Customer No." <> RequisitionLine."Sell-to Customer No.") or
                                                                                          // SGH             (PrevShipToCode <> RequisitionLine."Ship-to Code") or
              ...
              // SGH Add criteria to check if SO No. has changed, then return True
              (PrevSONo <> RequisitionLine."Sales Order No.") or
        ...
        // SGH Message('CheckInsertFinalizePurchaseOrderHeader() returned %1 (true = new PO) for SO no. %2', CheckInsert, RequisitionLine."Sales Order No.");
        exit(CheckInsert); // If true then new PO is created
    end;
